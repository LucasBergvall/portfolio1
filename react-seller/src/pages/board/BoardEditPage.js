  ...
  // 2. 이펙트
    ...
    const getinfo = async() => {
      ...
      try {
        ...
        const {data} = await axios.get(`/api2/board/select?no=${no}`, {headers})
        console.log(data);
    ...
    const handleImageDelete = async (imgNo) => {
      ...
      try {
        ...
        const { data } = await axios.delete('/api2/actionboard/delete', { headers: headers, data: body });
    ...
    const onFinish = async (e) => {
      ...
      try {
        ...
        const board = {"no": no, "title": title, "text": content}
        ...
        const headers = {Authorization: `Bearer ${token}`};
        const {data} = await axios.put(`/api2/board/update`, formData ,{headers})
        console.log(data);
        if(data.status === 1) {
          handleToggle();
          setPreviewUrls([]);
