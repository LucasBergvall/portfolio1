import React, { useEffect, useState } from 'react';
import { Form, Input, Button, Card, message, Layout, Menu } from 'antd';
import { Link, useNavigate } from 'react-router-dom';
import { ShoppingCartOutlined, UserOutlined, FileTextOutlined, FileSearchOutlined, CloudUploadOutlined, CloudServerOutlined, } from '@ant-design/icons';
import './css/ChangePw.css';
import { useDispatch, useSelector } from 'react-redux';
import axios from 'axios';

const ChangePw = () => {
  // 1. 변수
  const [form] = Form.useForm();
  const [loading, setLoading] = useState(false);
  const { Sider } = Layout;
  const { token } = useSelector((state) => state.LoginReducer);
  const navigate = useNavigate();
  const dispatch = useDispatch();

  // 2. 이펙트
    useEffect(() => {
      // 페이지에 처음 진입할 때 한 번 dispatch 실행
      dispatch({ type: 'mypage_menu', idx: "3" });
    }, [dispatch]);


    useEffect(() => {
      
    }, []);

    
  // 3. 함수
  const handleFinish = async(values) => {
    setLoading(true);

    const updatedFields = {};
    Object.keys(values).forEach((key) => {
      if (values[key]) {
        updatedFields[key] = values[key];
      }
    });

    if (Object.keys(updatedFields).length === 0) {
      message.warning('변경할 내용을 입력해주세요.');
      setLoading(false);
      return;
    }

    if(values.newpassword ==! values.newpasswordcheck) {
      message.warning('새 비밀번호가 일치하지 않습니다.');
      setLoading(false);
      return;
    }

    const url = `/api2/member/changepassword`
    const headers = { Authorization: `Bearer ${token}` }
    const body = {"currentpassword": values.currentpassword, "newpassword": values.newpassword};
    const {data} = await axios.put(url, body, {headers});
      console.log(data)
      if(data.status === 1) {
        setTimeout(() => {
          message.success('비밀번호가 변경되었습니다.');
          form.resetFields();
          setLoading(false);
          navigate('/mypage/home')
          dispatch({type: 'mypage_menu', idx : "0"});
        }, 1000);
      } else {
        setTimeout(() => {
          message.error('현재 비밀번호가 일치하지 않습니다.');
          form.resetFields();
          setLoading(false);
        }, 1000);
      }

    } 

  return (
      <div className="profile-container">
        <Card title="회원정보 관리" className="profile-card">
          <Form
            form={form}
            layout="vertical"
            onFinish={handleFinish}
          >
            <Form.Item label="현재 비밀번호" name="currentpassword">
              <Input.Password placeholder="현재 비밀번호 입력" iconRender={() => null} />
            </Form.Item>

            <Form.Item label="새 비밀번호" name="newpassword" >
              <Input.Password placeholder="새 비밀번호 입력" iconRender={() => null} />
            </Form.Item>

            <Form.Item label="새 비밀번호 확인" name="newpasswordcheck" >
              <Input.Password placeholder="새 비밀번호 확인" iconRender={() => null} />
            </Form.Item>

            <Form.Item>
              <Button className="profile-btn" type="primary" htmlType="submit" loading={loading} style={{background: '#F7C600', color: '#000000'}}>
                수정하기
              </Button>
            </Form.Item>
          </Form>
        </Card>
      </div>
  );
};

export default ChangePw;